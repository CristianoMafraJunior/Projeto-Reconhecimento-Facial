...
const usuarioReconhecido = localStorage.getItem('usuarioReconhecido');
if (usuarioReconhecido) {
    const nomeUsuarioElemento = document.getElementById('nome-usuario');
    if (nomeUsuarioElemento) {
        nomeUsuarioElemento.innerText = `Bem-vindo, ${usuarioReconhecido}!`;
    } else {
        console.error('Elemento com ID "nome-usuario" não encontrado.');
    }
} else {
    console.error('Nenhum usuário reconhecido encontrado no localStorage.');
}
